...
def _migrate_column_to_enum(conn, table_name, column_name, enum_type, migration_sql, default_value):
    """Migrate a string column to enum type"""
    logger.info(f"Migrating {table_name}.{column_name} to enum...")
    try:
        ...
        if row and 'enum' not in row[0].lower():
            ...
            logger.info(f"✅ {table_name}.{column_name} migrated to enum")
        else:
            logger.info(f"✅ {table_name}.{column_name} already enum type")
    except Exception as e:
        logger.error(f"❌ Error migrating {table_name}.{column_name}: {e}")
        raise


def _add_string_length_constraints(conn):
    """Add string length constraints to tables"""
...
def _add_check_constraints(conn):
...
def upgrade():
    ...
    with engine.begin() as conn:
        ...
        _migrate_column_to_enum(
            conn, 'signals', 'action', 'signalaction',
            """
                ALTER TABLE signals 
                ...
                ALTER TABLE signals DROP COLUMN IF EXISTS action CASCADE;
                ALTER TABLE signals RENAME COLUMN action_enum TO action;
                ALTER TABLE signals ALTER COLUMN action SET NOT NULL;
            """,
            'BUY'
        )
        ...
        _migrate_column_to_enum(
            conn, 'notifications', 'type', 'notificationtype',
            """
                ALTER TABLE notifications 
        ...
        _migrate_column_to_enum(
            conn, 'backtests', 'status', 'backteststatus',
            """
                ALTER TABLE backtests 
                ...
                ALTER TABLE backtests DROP COLUMN IF EXISTS status CASCADE;
                ALTER TABLE backtests RENAME COLUMN status_enum TO status;
                ALTER TABLE backtests ALTER COLUMN status SET NOT NULL;
            """,
            'running'
        )
        
        # Add string length constraints
        _add_string_length_constraints(conn)
