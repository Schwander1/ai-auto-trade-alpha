"""
Argo Signal Sync API Endpoint
Receives signals from Argo Capital and stores them in Alpine Analytics database
...
router = APIRouter(prefix="/api/v1/argo", tags=["argo-sync"])


class ArgoSignalRequest(BaseModel):
    """Signal data from Argo Capital"""
    signal_id: str
    symbol: str
    action: str  # BUY/SELL
    entry_price: float
...
def verify_argo_api_key(x_api_key: Optional[str] = Header(None, alias="X-API-Key")) -> bool:
    """
    Verify Argo API key for secure signal sync
    Uses HMAC or API key authentication
    """
    # Get API key from settings (AWS Secrets Manager or env)
    expected_key = getattr(settings, 'ARGO_API_KEY', None)
    if not expected_key:
        # Try to get from secrets manager
        try:
            if hasattr(settings, 'secrets') and settings.secrets:
                expected_key = settings.secrets.get_secret("argo-api-key", service="alpine-backend")
        except Exception as e:
            logger.debug(f"Could not get Argo API key from secrets: {e}")
    
    # If no key configured, allow but log warning (for development)
    if not expected_key:
        logger.warning("⚠️  Argo API key not configured - allowing requests (development mode)")
...
async def receive_signal_from_argo(
    signal_data: ArgoSignalRequest,
    db: Session = Depends(get_db),
    _: bool = Depends(verify_argo_api_key)
):
    """
    Receive and store signal from Argo Capital
    
    This endpoint maintains separation:
    - Argo Capital sends signals via API
    - Alpine Analytics stores in its own database
    - No direct database access between entities
    
    **Authentication:**
    - Requires X-API-Key header with valid Argo API key
    
    **Example Request:**
    ```bash
    curl -X POST "http://localhost:9001/api/v1/argo/sync/signal" \
         -H "X-API-Key: your-argo-api-key" \
    ...
    try:
        ...
        logger.info(f"✅ Signal synced from Argo: {signal_data.symbol} {signal_data.action} ({signal.id})")
        ...
        return {
            ...
            "argo_signal_id": signal_data.signal_id
        }
    
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"❌ Error storing signal from Argo: {e}")
        db.rollback()
        raise HTTPException(
            status_code=500,
...
async def sync_health():
    """Health check for Argo sync endpoint"""
    return {
        "status": "healthy",
