...
- Added agentic features troubleshooting section
- Added agentic deployment troubleshooting
- Added agentic cost monitoring troubleshooting
...
### Issue Category 8: Agentic Features Issues

#### Issue: Copilot CLI not found

**Symptoms**: `copilot: command not found`, wrapper script fails

**Solutions**:
1. Install: `npm install -g @githubnext/github-copilot-cli`
2. Add to PATH: `export PATH="/Users/dylanneuenschwander/.nvm/versions/node/v20.11.0/bin:$PATH"`
3. Create symlink: `ln -sf github-copilot-cli copilot` in npm bin directory

**See**: `docs/SystemDocs/AGENTIC_FEATURES_COMPLETE_GUIDE.md` for complete troubleshooting

---

#### Issue: Agentic deployment fails

**Symptoms**: Agentic deployment command fails, safety gates not executed

**Solutions**:
1. Make script executable: `chmod +x scripts/agentic/copilot-with-rules.sh`
2. Verify authentication: `copilot diagnostic`
3. Fallback to traditional deployment if needed
...
**Symptoms**: Unexpected API costs, rate limits exceeded

**Solutions**:
1. Check usage: `pnpm agentic:usage`
2. Enable rate limiting in `scripts/agentic/rate_limiter.py`
3. Enable caching with `cached_claude.py`
