#!/bin/bash
# Comprehensive fix for ALL old color references

echo "🎨 Fixing ALL remaining color references comprehensively..."
...
update_file() {
  ...
  # Update old color names to brand colors - comprehensive
  sed -i '' \
...
find "$COMPONENT_DIR" "$APP_DIR" -name "*.tsx" -type f ! -name "*.bak*" | while read file; do
  if grep -qE "\b(alpine-accent|alpine-pink|alpine-blue|alpine-darker|alpine-card|alpine-border|alpine-text-dim|alpine-text|alpine-red|alpine-green|alpine-bg)\b" "$file"; then
    update_file "$file"
  fi
done

echo ""
echo "✅ All color references fixed comprehensively!"
