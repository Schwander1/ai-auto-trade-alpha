#!/bin/bash
# Generate InvestorDocs v2.0 PDF with emoji removal and proper formatting
...
DATE=$(date +%Y-%m-%d)
TEMP_DIR=$(mktemp -d)
OUTPUT_PDF="pdfs/InvestorDocs_v2.0_${DATE}.pdf"

echo "Generating InvestorDocs v2.0 PDF..."
echo "=================================="
echo ""
...
for file in "$DOCS_DIR"/v2.0_*.md; do
    if [ -f "$file" ]; then
...
pandoc "$TEMP_DIR"/v2.0_*.md -o "$OUTPUT_PDF" \
  --pdf-engine=pdflatex \
  --toc \
  --toc-depth=3 \
