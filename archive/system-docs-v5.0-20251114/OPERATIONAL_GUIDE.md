...
Production uses blue-green deployment. To deploy:

```bash
# Deploy using blue-green (zero-downtime, recommended)
./scripts/deploy-argo-blue-green.sh

# Or test deployment first (safe, doesn't switch traffic)
...
# View active environment logs
ssh root@178.156.194.174 "tail -f /tmp/argo-{blue|green}.log"
...
```bash
# Deploy using blue-green (zero-downtime)
